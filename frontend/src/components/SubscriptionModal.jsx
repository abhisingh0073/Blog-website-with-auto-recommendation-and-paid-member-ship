...
export const SubscriptionModal = ({ isOpen, onClose, creatorId }) => {
  ...
  return (
    ...
    >
      <div
        className="relative w-full max-w-3xl mx-4 rounded-2xl bg-[var(--bg-surface)] p-8 shadow-2xl"
        onClick={(e) => e.stopPropagation()}
      >
       
        <button
          onClick={onClose}
          className="absolute right-4 top-4 text-[var(--text-secondary)] hover:text-[var(--text-primary)] cursor-pointer"
          aria-label="Close"
        >
          ✕
        </button>
        ...
        <div className="grid gap-6 md:grid-cols-3">
          {/* Plan 1 */}
          <div className="flex flex-col items-center rounded-xl border-2 border-[var(--border-color)] bg-[var(--bg-color)] p-6 text-center shadow-sm transition hover:shadow-md hover:border-indigo-600">
            <h3 className="text-lg font-semibold text-[var(--text-primary)]">1 Month</h3>
            <p className="mt-2 text-sm text-gray-500">
              Perfect for trying things out
            </p>
            <div className="my-4 text-3xl font-bold text-indigo-600">₹199/Month</div>
            <button className="w-full rounded-lg bg-indigo-600 px-4 py-2 font-semibold text-[var(--text-primary)] transition hover:bg-indigo-700 cursor-pointer"
          ...
          {/* Plan 2 (Highlighted) */}
          <div className="flex flex-col items-center rounded-xl border-2 border-[var(--border-color)] bg-[var(--bg-color)] p-6 text-center shadow-md transition hover:shadow-lg hover:border-indigo-600">
            <span className="mb-2 rounded-full bg-indigo-100 px-3 py-1 text-xs font-semibold text-indigo-700">
              Most Popular
            </span>
            <h3 className="text-lg font-semibold text-[var(--text-primary)]">3 Months</h3>
          ...
          {/* Plan 3 */}
          <div className="flex flex-col items-center rounded-xl border-2 border-[var(--border-color)] bg-[var(--bg-color)] p-6 text-center shadow-sm transition hover:shadow-md hover:border-indigo-600">
            <h3 className="text-lg font-semibold text-[var(--text-primary)]">6 Months</h3>
            <p className="mt-2 text-sm text-gray-500">
